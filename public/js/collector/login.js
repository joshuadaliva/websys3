  ...
  setTimeout(() => {
    ...
    if ((email === "admin" || email.includes("@")) && pw.length >= 6) {
      ...
      setTimeout(() => {
        ...
        window.location.href = "arkipaisi-admin-dashboard.html";
      }, 1200);
    } else {
      renderLogin({
        type: "err",
...
<input class="fc has-icon" id="inp-forgot-email" type="email" placeholder="admin@lgu.gov.ph" autocomplete="email" onkeydown="if(event.key==='Enter')doForgot()"/>
</div>
</div>
...
<div class="form-sub">Choose a strong password for your admin account.</div>
